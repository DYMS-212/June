"""
half_layer_tfim_generator.py - 基于Half-Layer的TFIM模型量子电路生成器
-------------------------------------------------------------------------------
* 基于海森堡模型生成器，专门用于TFIM（横向场伊辛模型）
* 第一层自动添加一整层H门
...
# TFIM默认门类型映射（包含H门）
DEFAULT_TFIM_GATE_MAPPING = {
    0: "h",     # H门（Hadamard门）- 用于初始层
    1: "rx",    # Rx门
    2: "ry",    # Ry门
    3: "rz",    # Rz门
    4: "rxx",   # Rxx门
...
class HalfLayerTFIMGenerator:
    """基于Half-Layer的TFIM模型量子电路生成器"""
    
    def __init__(self, 
                 gate_mapping: Optional[GateMapping] = None,
                 num_single_gates: int = 3,  # 不包括H门的单量子比特门数量
                 logger=None,
                 log_file=None, 
        ...
        参数：
            ...
            num_single_gates: 单量子比特门的数量（不包括H门）
            logger: 日志器实例
            log_file: 日志文件路径
            log_level: 日志级别
        """
        # 设置门类型映射
        self.gate_mapping = gate_mapping or DEFAULT_TFIM_GATE_MAPPING
        ...
        if logger is not None:
            ...
            self.logger = logging.getLogger("HalfLayerTFIMGenerator")
    ...
    def _generate_gate_probabilities(self, 
        ...
        """
        生成门类型的概率分布（不包括H门，因为H门只在第一层使用）
        
        参数：
            gates_count: 门类型的总数（不包括H门）
    ...
    def _create_initial_h_layer(self, num_qubits: int) -> Layer:
        """
        创建初始的H门层，覆盖所有量子比特
        ...
        for q in range(num_qubits):
            h_layer.append((0, q, q))  # H门类型为0，单量子比特门
        return h_layer
    ...
    def generate_tfim_half_layer_circuit(self,
                                        num_qubits: int,
                                        max_gates: int,
                                        max_add_count: int,
        ...
        """
        基于Half-Layer生成TFIM量子电路（第一层为H门层）
        
        参数：
            num_qubits: 量子比特数量
            max_gates: 最大门层数（不包括初始H层）
            max_add_count: 最多添加的门数量（不包括初始H层）
        ...
        rng = rng or np.random.default_rng()
        # 门类型1-6（不包括H门）
        gates_count = len(self.gate_mapping) - 1  # 减去H门
        ...
        # 第一步：添加初始H门层
        initial_h_layer = self._create_initial_h_layer(num_qubits)
        layers.append(initial_h_layer)
        self.logger.info(f"Added initial H layer with {len(initial_h_layer)} H gates")
        
        # 初始化计数器和历史记录
        add_count = num_qubits  # 已经添加了H门
        step = 1  # 已经有了H层
        
        # 记录前一层，用于相邻层重复检测
        prev_layer = initial_h_layer.copy()
        
        # 记录每个量子比特位置的门类型历史
        qubit_gate_history = {}
        # 更新H门的历史记录
        self._update_gate_history(initial_h_layer, qubit_gate_history)
        
        # 逐层生成电路（从第二层开始）
        while step < max_gates + 1 and add_count < max_add_count + num_qubits:  # +num_qubits因为H门
            current_layer = []
            # 用于跟踪当前层已占用的量子比特
            occupied_qubits = set()
            ...
            for pos in positions:
                # 如果已经达到最大门数，跳出循环
                if add_count >= max_add_count + num_qubits:  # +num_qubits因为H门
                    break
        ...
        self.logger.info(f"Generated TFIM circuit with {add_count} gates in {len(layers)} layers (including initial H layer)")
        return layers
    ...
    def layers_to_qiskit(self, layers: Layers) -> QuantumCircuit:
        ...
        for layer in layers:
            for gate_type, q1, q2 in layer:
                ...
                # 添加门
                if gate_type == 0:  # H门不需要参数
                    qc.h(q1)
                elif gate_type <= self.num_single_gates:  # 其他单量子比特门
    ...
    def generate_tfim_quantum_circuits(self, 
        ...
        self.logger.info(f"Starting to generate {num_circuits} TFIM quantum circuits...")
        
        while len(circuits) < num_circuits:
            # 生成层结构
            layers = self.generate_tfim_half_layer_circuit(
            ...
            if len(circuits) % 1000 == 0:
                self.logger.info(f"Generated {len(circuits)}/{num_circuits} TFIM circuits.")
        
        self.logger.info("All TFIM circuits generated successfully.")
        return circuits
    ...
    def visualize_layers(self, layers: Layers) -> None:
        ...
        for d, layer in enumerate(layers):
            for g, q1, q2 in layer:
                ...
                if g <= self.num_single_gates or g == 0:  # 单量子比特门（包括H门）
                    grid[q1][d] = sym
                else:  # 双量子比特门
    ...
    def save_circuits(self, circuits: List[QuantumCircuit], filename: str) -> None:
        ...
        参数：
            ...
            filename: 输出文件名
        """
        self.logger.info(f"Saving TFIM circuits to {filename}...")
        with open(filename, 'wb') as f:
            pickle.dump(circuits, f)
        self.logger.info("TFIM circuits saved successfully.")


# 使用示例
if __name__ == "__main__":
    # 创建TFIM生成器
    tfim_generator = HalfLayerTFIMGenerator(log_level=logging.INFO)
    
    # 生成单个TFIM电路层结构
    layers = tfim_generator.generate_tfim_half_layer_circuit(
        num_qubits=4,
        max_gates=5,
        max_add_count=20
    )
    ...
    # 可视化电路
    print("Generated TFIM circuit structure:")
    tfim_generator.visualize_layers(layers)
    
    # 转换为Qiskit电路
    qc = tfim_generator.layers_to_qiskit(layers)
    print(f"\nQiskit circuit:\n{qc}")
    
    # 生成多个TFIM电路
    circuits = tfim_generator.generate_tfim_quantum_circuits(
        num_circuits=10,
        num_qubits=4,
        max_gates=3,
        max_add_count=15
    ...
    print(f"Generated {len(circuits)} unique TFIM circuits")
